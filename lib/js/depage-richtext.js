        ...
        $.extend(this, {
            ...
            writeDocument : function() {
                ...
                documentTemplate = ($.browser.msie) ? 
                    documentTemplate.replace(/INSERT:STYLESHEET:END/, '<link rel="stylesheet" type="text/css" href="' + this.depageEditorStylesheet + '"></link>') :
            ...
            makeEditable : function() {
                ...
                $(this.iframe.contentWindow.document).keydown(function(e){ 
                    self.detectPaste(e); 
                });
                $(this.iframe.contentWindow.document).scroll(function() { 
                    self.autogrow();
                });
                this.locked = false;
            },
...
*/    
            ...
            autogrow : function() {
                if (this.wysiwyg) {
                    var extraSpace = 40;
                    var innerBody = $(this.iframe.contentWindow.document).find("body");
                    var newHeight = $.browser.msie ? innerBody[0].scrollHeight : innerBody.height() + extraSpace; 
        ...
        $.extend(this, {
            ...
            checkState : function(depageEditor, resubmit) {
                if (!resubmit) {
                    ...
                    setTimeout(function(){depageEditor.toolbar.checkState(depageEditor, true); return true;}, 500);
                    return true;
                }
                ...
                /* IE selections */
                if (depageEditor.iframe.contentWindow.document.selection) {
                    selection = depageEditor.iframe.contentWindow.document.selection;
                ...
                /* Mozilla selections */
                    ...
                    try {
                        selection = depageEditor.iframe.contentWindow.getSelection();
                    }
                    catch (e) {
                        return false;
                    }
                ...
                while (!parentnode.is('body')) {
                    if(parentnode.is('a')) depageEditor.toolbar.setState("link", "on");
                    else if(parentnode.is('em'))depageEditor.toolbar.setState("italic", "on");
                    else if(parentnode.is('strong')) depageEditor.toolbar.setState("bold", "on");
                    ...
                    else depageEditor.toolbar.setState("formatblock", parentnode[0].nodeName.toLowerCase());
                    parentnode = parentnode.parent();
                }                        
            },
            // }}}
            ...
            setState: function(state, status) {
                if (state != "SelectBlock") $(this.itemsList).find('.' + $.depageEditorToolbarItems[state].className).addClass('on');
                else $(this.itemsList).find('.' + $.depageEditorToolbarItems[state].className).val(status);            
            },
            // }}}
            // {{{ init()
    ...
    var depageEditorToolbarItems = function() {
        ...
        if(typeof(depageEditorToolbarItemsClass.singleton) != 'undefined') return depageEditorToolbarItemsClass.singleton;
        else depageEditorToolbarItemsClass.singleton = this;
