...
var tableNodeNames  = /^(T(R|D|H|BODY|HEAD|ABLE))$/;
...
function getNodeCategory ( node ) {
    ...
    var nodeCategory;
    if ( tableNodeNames.test( node.nodeName ) ) {
        nodeCategory = TABLE;
...
function isTable ( node ) {
    return getNodeCategory( node ) === TABLE;
}
...
var moveRangeBoundariesUpTree = function ( range, startMax, endMax, root ) {
    ...
    while ( !startOffset &&
            ...
            startContainer !== root &&
            !isTable(startContainer.parentNode)) {
        parent = startContainer.parentNode;
    ...
    while ( true ) {
        ...
        if (endContainer === endMax ||
            endContainer === root ||
            endOffset !== getLength( endContainer ) ||
            isTable(endContainer.parentNode)
        ) {
            break;
        }
...
var cleanupBRs = function ( node, root, keepForBlankLine ) {
    ...
    while ( l-- ) {
        ...
        if ( !brBreaksLine[l] ) {
            detach( br );
        } else if ( !isInline( parent ) && parent.nodeName != root.__squire__._config.blockTag ) {
